...
def test_create_task(repo_task, add_task):
    task_data = {
        "title": "Test task",
...
def test_update_task(repo_task, add_task):
    task_data = {
        "is_done" : True,
        "deadline" : datetime(2025, 12, 10, 13, 45),
        "title" : "test",
...
def test_del_exists_task(repo_task, add_task):
...
def test_get_all_tasks(repo_task, add_task):
    tasks = repo_task.get_all().all()
    assert len(tasks) > 0
    assert tasks[0].title == "Test task"

def test_get_isdone_tasks(repo_task, add_task):
    done_tasks = repo_task.get_isdone(False).all()
    assert all(not t.is_done for t in done_tasks)

def test_get_user_tasks_by_id(repo_task, add_task):
    task_data = {
        "title": "Test task",
        "description": "Test description",
        "is_done": False,
    ...
    tasks = repo_task.get_user_tasks(user_id=9)
    assert len(tasks) == 1
    assert tasks[0].owner_id == 9
    assert tasks[0].title == "Test task"

def test_partial_update_task(repo_task, add_task):
    data = {
        "title" : "Updated title", 
        "description" : None
    }
...
def test_get_user_tasks_with_deadline(repo_task, add_task):
    deadline = datetime(2025, 12, 10, 13, 45)
...
def test_create_user_rollback_on_duplicate(repo_user, add_user):
    user = {
        "name" : "Test",
        "email" : "dontest@exam.com",  # уже есть
        "password_hash" : "hash"
    }        
    with pytest.raises(NotUniqEmail):
        repo_user.create_user(user)
